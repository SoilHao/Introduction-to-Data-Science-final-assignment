...
rate=pd.read_table('ratings.dat')
# 假设您的DataFrame名为df，列名为'column'
rate[['userId','movieId','rating','timestamp']] = rate['1::122::5::838985046'].str.split('::', expand=True)
movies[['movieId','title','genres']] = movies['1::Toy Story (1995)::Adventure|Animation|Children|Comedy|Fantasy'].str.split('::', expand=True)
rate['rating'] = pd.to_numeric(rate['rating'])
rate['userId'] = pd.to_numeric(rate['userId'])
rate['movieId'] = pd.to_numeric(rate['movieId'])
