...
rate=pd.read_csv('20m.ratings.csv')
# 合并数据
...
#认为优评过少不足以进入推荐电影的名单，优评次数大于12000次的电影视作推荐电影
filtered_values=movie_counts[movie_counts>12000].index
#根据筛选后的值构建新表
...
#将DataFrame转换为字典形式，统计每个用户更推荐哪些电影（优评数大于12000次）
...
for i in [0, 4]:
    ...
    plt.annotate(f"{y_bias_svd_rmse[i]:.3f}", (x[i], y_bias_svd_rmse[i]), textcoords="offset points", xytext=(0,3), ha='center', color='orange', fontsize=9)
...
for i in [0, 4]:
    ...
    plt.annotate(f"{y_bias_svd_mae[i]:.3f}", (x[i], y_bias_svd_mae[i]), textcoords="offset points", xytext=(0,2), ha='center', color='orange', fontsize=8)
...
# 假设你想为用户推荐10部电影
userId=4
recommended_movies = recommend_movies(userId, all_user_predicted_ratings,10)
